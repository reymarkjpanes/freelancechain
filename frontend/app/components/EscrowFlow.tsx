...
export default function EscrowFlow({
  ...
  if (loading) {
    return (
      <div className="step-card text-center py-8">
        <Spinner />
        <p className="text-sm text-slate-500 mt-2">Loading job state from chain…</p>
      </div>
    );
  }
  ...
  return (
    <div className="step-card space-y-5">
      ...
      <div className="bg-slate-50 rounded-xl border border-slate-200 px-4 py-3 text-xs space-y-1.5 font-mono">
        ...
        <Row label="Client" value={truncate(clientAddress)} isYou={isClient} />
        <Row label="Freelancer" value={truncate(freelancerAddress)} isYou={isFreelancer} />
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    {isActive && userCanAct && (
                      <span className="badge bg-violet-100 text-violet-700 animate-pulse">
                        ← Your turn
                      </span>
                    )}
                  </div>
                  ...
                  {wrongWallet && (
                    <p className="text-xs text-amber-600 mt-1.5 flex items-center gap-1">
                      <span>⚠</span>
                      Switch to the {step.requiredRole === "Client" ? "Client" : "Freelancer"} wallet
                      ({truncate(step.requiredRole === "Client" ? clientAddress : freelancerAddress)}) to proceed.
                    </p>
                  )}
                </div>
...
function Row({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={highlight ? "font-bold text-violet-700" : "text-slate-700"}>
        {value}
      </span>
      {isYou && (
        <span className="badge text-[10px] bg-violet-100 text-violet-600">you</span>
      )}
    </div>
  );
}
